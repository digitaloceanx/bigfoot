...
function addon:Initialize()
    ...
    self:RegisterEvent("PLAYER_SPECIALIZATION_CHANGED", "TalentGroupChanged")
...
function addon:GetClickAttributes(global)
    ...
    for idx, entry in ipairs(self.bindings) do
        ...
        if shouldApply(global, entry) and correctSpec(entry, GetSpecialization()) and entry.key then
...
function addon:GetBindingAttributes(global)
    ...
    for idx, entry in ipairs(self.bindings) do
		if entry.key then
			if shouldApply(global, entry) and correctSpec(entry, GetSpecialization()) then
				if global then
...
function addon:TalentGroupChanged()
	...
	if self.settings.specswap then
		self.talentGroup = GetSpecialization()
